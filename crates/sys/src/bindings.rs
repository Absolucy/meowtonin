...
impl ByondApi {
		...
		L: Into<::libloading::Library>,
	{
		...
		let ByondValue_DecTempRef = __library.get(b"_ByondValue_DecTempRef\0").map(|sym| *sym)?;
	...
	#[doc = "Creates a string and sets CByondValue to a reference to that string, and increases \
	         the reference count. See REFERENCE COUNTING in byondapi.h.\n Blocks if not on the \
	         main thread. If string creation fails, the struct is set to null.\n # Arguments\n\n* \
	...
	#[doc = "Fills a CByondValue struct with a reference to a string with a given ID. Does not \
	         validate, and does not increase the reference count.\n If the strid is NONE, it will \
	         be changed to 0.\n # Arguments\n\n* `v` - Pointer to CByondValue\n * `strid` - 4-byte \
	         string ID\n [`Byond_TestRef()`]"]
	...
	#[doc = "Runs a function as a callback on the main thread (or right away if already there)\n \
	         All references created from Byondapi calls within your callback are persistent, not \
	         temporary, even though your callback runs on the main thread.\n Blocking is optional. \
	         If already on the main thread, the block parameter is meaningless.\n # Arguments\n\n* \
	         ...
	         waiting for the callback to finish; false if not\n # Returns\n\nCByondValue returned \
	         by the function (if it blocked; null if not)"]
	...
	#[doc = "Returns a reference to an existing string ID or creates a new string ID reference.\n \
	         The new string is reference-counted. See REFERENCE COUNTING in byondapi.h for \
	...
	#[doc = "Reads items (referenced) from a list.\n Blocks if not on the main thread.\n # \
	         ...
	         caller (can be null if querying length)\n * `len` - Pointer to length of array (in \
	         items); receives the number of items read on success, or required length of array if \
	         not big enough\n # Returns\n\nTrue on success; false with *len=0 for failure; false \
	         with *len=required size if array is not big enough"]
	...
	#[doc = "Reads items as key,value pairs from an associative list, storing them sequentially as \
	...
	#[doc = "Reads an item (referenced) from a list.\n Blocks if not on the main thread.\n # \
	         ...
	         or a non-number if using associative lists)\n * `result` - Pointer to accept result\n \
	         # Returns\n\nTrue on success"]
	...
	#[doc = "Uses BYOND's internals to represent a value as text\n Blocks if not on the main \
	         ...
	         array, allocated by caller (can be null if querying length)\n * `buflen` - Pointer to \
	         length of array in bytes; receives the string length (including trailing null) on \
	         success, or required length of array if not big enough\n # Returns\n\nTrue on \
	         success; false with *buflen=0 for failure; false with *buflen=required size if array \
	         is not big enough"]
	...
	#[doc = "Equivalent to calling block(x1,y1,z1, x2,y2,z2).\n Blocks if not on the main \
	         ...
	         be null if querying length)\n * `len` - Pointer to length of array (in items); \
	         receives the number of items read on success, or required length of array if not big \
	...
	#[doc = "Equivalent to calling length(src).\n Blocks if not on the main thread.\n # \
	         ...
	         CByondValue (intended for future possible override of length)\n # Returns\n\nTrue on \
	         success"]
	...
	#[doc = "Equivalent to calling locate(type), or locate(type) in list.\n Blocks if not on the \
	         ...
	         locate in; can be a null pointer instead of a CByondValue to locate(type) without a \
	...
	#[doc = "Increase the persistent reference count of an object used in Byondapi\n Reminder: \
	         Calls only create temporary references when made on the main thread. On other \
	         threads, the references are already persistent.\n Blocks if not on the main thread.\n \
	...
	#[doc = "Mark a persistent reference as no longer in use by Byondapi\n This is IMPORTANT to \
	         call when you make Byondapi calls on another thread, since all the references they \
	         create are persistent.\n This cannot be used for temporary references. See \
	         ByondValue_DecTempRef() for those.\n Blocks if not on the main thread.\n # \
	...
	#[doc = "Mark a temporary reference as no longer in use by Byondapi\n Temporary references \
	         will be deleted automatically at the end of a tick, so this only gets rid of the \
	         reference a little faster.\n Only works on the main thread. Does nothing on other \
	         threads.\n # Arguments\n\n* `src` - The object to decref"]
